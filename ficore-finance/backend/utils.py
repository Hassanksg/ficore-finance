...
class SessionFormatter(logging.Formatter):
    def format(self, record):
        ...
        record.jti = getattr(record, 'jti', 'no_jti')
    ...
    def process(self, msg, kwargs):
        ...
        try:
            if has_request_context():
                ...
                try:
                    jwt_data = get_jwt()
                    jti = jwt_data.get('jti', 'no_jti') if jwt_data else 'no_jti'
                except:
                    jti = 'no_jti'
            else:
        ...
        kwargs['extra']['jti'] = jti
        return msg, kwargs

logger = SessionAdapter(root_logger, {})

# Tool/navigation lists with endpoints for web and mobile
_PERSONAL_TOOLS = [
    {
...
def get_explore_features():
    ...
    try:
        with current_app.app_context():
            ...
            required_keys = ['endpoint', 'route', 'label', 'label_key', 'description_key', 'tooltip_key', 'icon']
...
def initialize_tools_with_urls(app):
    """
    Initialize all tool/navigation lists with resolved URLs for web and mobile.
...
def generate_tools_with_urls(tools):
    """
    Generate a list of tools with resolved URLs and validated icons for web and mobile.
    
    Args:
        tools: List of dictionaries containing 'endpoint', 'route', and 'icon' keys
    ...
    for tool in tools:
        try:
            # Use route for mobile, fallback to endpoint for web
            if 'route' in tool:
                url = url_for('general_bp.api_route', path=tool['route'].lstrip('/'), _external=True)
            else:
                url = url_for(tool['endpoint'], _external=True)
            icon = tool.get('icon', 'ion-question-circle')
            if not icon or not (icon.startswith('bi-') or icon.startswith('ion-')):
                logger.warning(f"Invalid or missing icon for tool {tool.get('label', 'unknown')}: {icon}. Using fallback 'ion-question-circle'.")
                icon = 'ion-question-circle'
            result.append({**tool, 'url': url, 'icon': icon})
        except BuildError as e:
            logger.warning(f"Failed to generate URL for endpoint {tool.get('endpoint', 'unknown')} or route {tool.get('route', 'unknown')}: {str(e)}. Ensure endpoint or route is defined in blueprint.")
            result.append({**tool, 'url': '#', 'icon': tool.get('icon', 'ion-question-circle')})
        except RuntimeError as e:
            logger.warning(f"Runtime error generating URL for endpoint {tool.get('endpoint', 'unknown')} or route {tool.get('route', 'unknown')}: {str(e)}")
            result.append({**tool, 'url': '#', 'icon': tool.get('icon', 'ion-question-circle')})
    return result
...
def generate_jwt_token(user_id):
    ...
    try:
        ...
        logger.debug(f"Generated JWT token for user {user_id}", extra={'user_id': user_id})
        return token
    except Exception as e:
...
def check_jwt_blacklist(jti):
    ...
    try:
        db = get_mongo_db()
        token = db.jwt_blacklist.find_one({'jti': jti})
        is_blacklisted = token is not None and token.get('expires', datetime.utcnow()) > datetime.utcnow()
        logger.debug(f"JWT blacklist check for jti {jti}: {'blacklisted' if is_blacklisted else 'not blacklisted'}")
        return is_blacklisted
    except Exception as e:
        logger.error(f"Error checking JWT blacklist for jti {jti}: {str(e)}", exc_info=True)
        raise RuntimeError(f"Failed to check JWT blacklist: {str(e)}")

def get_jwt_identity_safe():
    """
    Safely get the JWT identity, handling cases where no JWT is present.
    
    Returns:
        str or None: User ID from JWT or None if not available
    """
    try:
        return get_jwt_identity()
    except Exception:
        logger.debug("No JWT identity available")
        return None
...
def log_tool_usage(action, tool_name=None, details=None, user_id=None, db=None, session_id=None):
    ...
    Args:
        action: The action performed (e.g., 'main_view', 'add_bill')
        tool_name: The name of the tool used (defaults to action if None)
    ...
    try:
        ...
        if not user_id:
            user_id = get_jwt_identity_safe() or (current_user.id if current_user.is_authenticated else None)
            if not user_id:
        ...
        log_entry = {
            ...
            'jti': get_jwt().get('jti', 'no_jti') if has_request_context() and get_jwt() else 'no_jti'
        }
...
def requires_role(role):
    """
    Decorator to require specific user role for both session and JWT-based auth.
            ...
            with current_app.app_context():
                ...
                if not user_id:
                    if request.accept_mimetypes.best == 'application/json':
                        return jsonify({'success': False, 'message': trans('general_login_required', default='Please log in to access this page.')}), 401
                ...
                user_role = current_user.role if current_user.is_authenticated else get_user_role(user_id)
                if user_role not in allowed_roles:
                    if request.accept_mimetypes.best == 'application/json':
                        return jsonify({'success': False, 'message': trans('general_access_denied', default='You do not have permission to access this page.')}), 403
...
def get_user_role(user_id):
    ...
    Returns:
        str: User role or 'personal' if not found
    """
    try:
        db = get_mongo_db()
        user = db.users.find_one({'_id': user_id})
        return user.get('role', 'personal') if user else 'personal'
    except Exception as e:
        logger.error(f"Error fetching user role for {user_id}: {str(e)}", exc_info=True)
        return 'personal'
...
def is_admin():
    """
    Check if current user is admin for both session and JWT-based auth.
    ...
    try:
        with current_app.app_context():
            ...
            db = get_mongo_db()
            user = db.users.find_one({'_id': user_id})
            return user and (user.get('role') == 'admin' or user.get('is_admin', False))
    except Exception:
        return False
...
def log_user_action(action, details=None, user_id=None):
    """
    Log user action for audit purposes with JWT support.
    
    Args:
        action: Action performed
        details: Additional details about the action
        user_id: User ID (optional, will use current_user or JWT if not provided)
    """
    try:
        with current_app.app_context():
            from flask_login import current_user
            ...
            logger.info(
                f"{trans('general_user_action_logged', default='User action logged')}: {action} by user {user_id}",
                extra={'user_id': user_id, 'session_id': session_id, 'jti': jti}
            )
    except Exception as e:
        logger.error(
            f"{trans('general_user_action_log_error', default='Error logging user action')}: {str(e)}",
            exc_info=True,
            extra={'user_id': user_id or 'unknown', 'session_id': session_id, 'jti': jti}
        )
...
def check_ficore_credit_balance(required_amount=1, user_id=None):
    """
    Check if user has sufficient Ficore Credits, supporting both session and JWT-based auth.
    
    Args:
        required_amount: Amount of credits required (default: 1)
        user_id: User ID (optional, uses current_user or JWT if not provided)
...
__all__ = [
    ...
    'generate_jwt_token', 'check_jwt_blacklist', 'get_jwt_identity_safe'
]
