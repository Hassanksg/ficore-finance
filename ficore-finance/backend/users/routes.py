...
from flask_jwt_extended import create_access_token, jwt_required, get_jwt_identity, get_jwt, jwt_optional
...
def api_login():
    try:
        ...
        if not data or 'username' not in data or 'password' not in data:
            logger.warning(f"API login failed: Missing username or password, session_id: {session.get('session_id')}")
            log_audit_action('api_login_failed', {'reason': 'missing_credentials'})
            return jsonify({'success': False, 'message': trans('general_identifier_required')}), 400

        form = LoginForm(data=data)
        if form.validate():
            identifier = form.username.data.strip().lower()
            password = form.password.data
            logger.info(f"API login attempt for identifier: {identifier}, session_id: {session.get('session_id')}")
            ...
            if not user:
                logger.warning(f"API login failed: Identifier {identifier} not found")
                log_audit_action('api_login_failed', {'identifier': identifier, 'reason': 'identifier_not_found'})
                return jsonify({'success': False, 'message': trans('general_identifier_not_found')}), 401

            username = user['_id']
            if not check_password_hash(user['password_hash'], password):
                logger.warning(f"API login failed for username: {username} (invalid password)")
                log_audit_action('api_login_failed', {'user_id': username, 'reason': 'invalid_password'})
                return jsonify({'success': False, 'message': trans('general_invalid_password')}), 401

            token = create_access_token(identity=username)
            log_audit_action('api_login', {'user_id': username})
            logger.info(f"API login successful for username: {username}, token issued")
            return jsonify({'success': True, 'token': token, 'setup_complete': user.get('setup_complete', False)}), 200
        else:
            logger.debug(f"API login form validation failed: {form.errors}")
            return jsonify({'success': False, 'errors': form.errors}), 400
    except errors.PyMongoError as e:
        logger.error(f"MongoDB error during API login: {str(e)}", exc_info=True)
        log_audit_action('api_login_failed', {'reason': 'mongodb_error', 'error': str(e)})
        return jsonify({'success': False, 'message': trans('general_database_error')}), 500
    except Exception as e:
        logger.error(f"Unexpected error during API login: {str(e)}", exc_info=True)
        log_audit_action('api_login_failed', {'reason': 'unexpected_error', 'error': str(e)})
        return jsonify({'success': False, 'message': trans('general_error')}), 500
...
def logout():
    ...
    prefer_json = request.accept_mimetypes.best == 'application/json'
    try:
        logout_user()
        ...
        if prefer_json:
            return jsonify({'success': True, 'message': trans('general_logged_out')}), 200
...
def api_logout():
    try:
        ...
        jti = get_jwt()['jti']
        db = utils.get_mongo_db()
        expiry = datetime.utcnow() + timedelta(hours=1)  # Match JWT_ACCESS_TOKEN_EXPIRES
        ...
        log_audit_action('api_logout', {'user_id': user_id, 'jti': jti})
        logger.info(f"API logout successful for user {user_id}, JTI {jti} blacklisted")
        return jsonify({'success': True, 'message': trans('general_logged_out')}), 200
    except errors.PyMongoError as e:
        logger.error(f"MongoDB error during API logout for user {user_id}: {str(e)}", exc_info=True)
        log_audit_action('api_logout_failed', {'user_id': user_id, 'reason': 'mongodb_error', 'error': str(e)})
        return jsonify({'success': False, 'message': trans('general_database_error')}), 500
    except Exception as e:
        logger.error(f"Unexpected error during API logout for user {user_id}: {str(e)}", exc_info=True)
        log_audit_action('api_logout_failed', {'user_id': user_id, 'reason': 'unexpected_error', 'error': str(e)})
        return jsonify({'success': False, 'message': trans('general_error')}), 500