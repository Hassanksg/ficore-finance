...
CORS(app, resources={r"/api/*": {"origins": "*"}}, supports_credentials=True)  # Allow all for dev; restrict in prod to mobile origins
app.config.from_mapping(
    ...
    JWT_SECRET_KEY=os.getenv('JWT_SECRET_KEY', 'super-secret-key'),  # Use env var in prod
    JWT_ACCESS_TOKEN_EXPIRES=timedelta(hours=1),  # Token expiration
)

# Validate critical configuration
for key in ['SECRET_KEY', 'MONGO_URI', 'ADMIN_PASSWORD', 'JWT_SECRET_KEY']:
...
def create_app():
    ...
    app.register_blueprint(budget_bp, url_prefix='/budget')  # Keep existing prefix, but routes will handle JSON/HTML
    ...
    with app.app_context():
        ...
        admin_email = os.getenv('ADMIN_EMAIL', 'admin@ficoreafrica.com')
